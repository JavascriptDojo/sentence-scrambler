...
function SentenceScrambler (options) {
    ...
    // Here, when the object is created, we're giving the calling code
    // the opportunity to pass in a dependency (the splitter).
    //
    // This is known as Dependency Injection, or DI.
    ...
    function add_sentence(string) {
        var sentenceArr = split(string);  //string.split(/\s+/);
